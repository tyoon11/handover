...
try:
    ...
    _HAS_TRL_SANITIZE = True
except ImportError:
    _HAS_TRL_SANITIZE = False
...
class SimPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args.model_init_kwargs is None:
            ...
            raise ValueError(
                "You passed model_kwargs to the SimPOTrainer. But your model is already instantiated."
            )
        else:
    ...
    def tokenize_row(
        ...
        if not self.is_encoder_decoder:
            ...
            # Qwen 계열(bos_token_id=None)은 BOS 미사용 — 건너뜀 (None을 input_ids에 넣으면
            # collator에서 torch.LongTensor(v) 실패함)
