...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 측정 오류 배제 (아티팩트) — **좁게** 잡는다
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 이 코호트는 소아 심장수술을 포함한다. 심폐순환우회·의도적 저체온(DHCA)에서
# 체온은 18~20°C까지 내려가고, 청색성 심질환·단심실에서 SpO2 40~70%가 실제로
# 관측된다. 따라서 "임상적으로 드문 값"을 지우면 진짜 소견을 삭제하게 된다.
# → **물리적으로 불가능하거나 센서 이탈로만 설명되는 값**만 버린다.
#   판단이 애매한 값은 남긴다 (남기면 이벤트로 보고될 뿐, 삭제는 되돌릴 수 없다).
PLAUSIBLE_RANGE = {
    "HR": (1.0, 300.0),        # >300 은 소아에서도 비생리적 (PALS 빈맥 상한 220)
    "SBP": (5.0, 300.0),       # <5 = 커프/라인 이탈
    "DBP": (5.0, 300.0),
    "MBP": (5.0, 300.0),
    "ISBP1": (5.0, 300.0),
    "IDBP1": (5.0, 300.0),
    "IMBP1": (5.0, 300.0),
    "SpO2": (1.0, 100.0),      # >100 은 정의상 불가 — 유일한 확실한 오류
    "T1": (14.0, 45.0),        # DHCA 18~20°C 는 보존. <14 = 프로브 이탈, >45 = 오류
    "QTc": (150.0, 800.0),
    "Ppeak": (1.0, 120.0),
}


def implausible_mask(item: str, values):
    """item 시계열에서 '측정 오류'로 버릴 값의 boolean mask. 범위 미정의면 전부 False."""
    rng = PLAUSIBLE_RANGE.get(item)
    if rng is None:
