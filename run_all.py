...
_RUN_ID = _early.run_id or os.environ.get("HANDOVER_RUN_ID") or datetime.now().strftime("%Y%m%d_%H%M%S")
os.environ["HANDOVER_RUN_ID"] = _RUN_ID
print(f"[RUN] HANDOVER_RUN_ID={_RUN_ID}")

import pandas as pd
from config import (
    SFT_MODELS, SFT_OUT, RLAIF_OUT, INFER_OUT, EVAL_OUT,
    SAMPLE_OUT, OUTPUT_BASE, WORK_DIR,
...
def make_sample_md(model: str, exp_key: str, n_samples: int = 5):
    ...
    md.append(f"**평균 점수**: Brevity={avg_b:.3f}  Critical={avg_c:.3f}  SUM={avg_b+avg_c:.3f}\n")
...
def summarize_results():
    ...
    for score_file in sorted(EVAL_OUT.rglob("*_scores.jsonl")):
        ...
        avg_b = sum(l.get("brevity_score",  0) for l in lines) / len(lines)
        avg_c = sum(l.get("critical_score", 0) for l in lines) / len(lines)
        rows.append(dict(model=model, experiment=exp_key, n=len(lines),
                         brevity=round(avg_b,3), critical=round(avg_c,3),
    ...
    for exp_key, sft_ep, rlaif_loss, sft_for_rlaif in exps:
        ...
        if not only_eval and sft_ep is not None:
            ...
            if skip_done and (sft_final / "config.json").exists():
                ...
                ok = run_cmd([py, "02_sft_train.py",
                              "--base", model, "--epochs", str(sft_ep)] + gpu_args,
                             f"SFT {sft_ep}ep")
                if not ok:
        ...
        if not only_eval and rlaif_loss is not None:
            ...
            if skip_done and (rlaif_final / "config.json").exists():
                ...
                cmd = [py, "03_rlaif_train.py",
                       "--base", model, "--loss", rlaif_loss] + gpu_args
                if sft_ckpt:
        ...
        if skip_done and infer_file.exists():
            ...
            cmd = [py, "04_inference.py",
                   "--model_path", ckpt,
                   "--split", "gold",
                   "--out_tag", f"{model}_{exp_key}"] + gpu_args
            if exp_key != "raw":
        ...
        if skip_done and eval_file.exists():
            ...
            if not infer_file.exists():
                log(f"  [SKIP] inference 결과 없음"); continue
            ok = run_cmd([py, "05_evaluate.py",
...
def main():
    ...
    parser.add_argument("--models", nargs="+", choices=list(SFT_MODELS.keys()),
                        default=["llama"])
