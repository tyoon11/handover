...
from config import (
    SPLIT_PKL, VITAL_MAP, SYNTH_PKL,
    SID_COL, AGE_COL, EMR_COLS,
    SYSTEM_PROMPT, USER_PROMPT_TEMPLATE, EVAL_RUBRICS,
    MAX_INPUT_LEN, MAX_OUTPUT_LEN, RANDOM_SEED, SYNTH_MODELS,
)
...
def generate_one(model, tokenizer, messages, device, max_new=MAX_OUTPUT_LEN) -> str:
    ...
    ids  = tokenizer(text, return_tensors="pt", truncation=True,
                     max_length=MAX_INPUT_LEN).input_ids.to(device)
    with torch.no_grad():
...
def score_response(judge_model, judge_tok, emr_text, response, reference, rubric_key, device):
    rubric = EVAL_RUBRICS[rubric_key]
    rubric_str = rubric["criteria"] + "\n" + "\n".join(
        f"Score {k}: {v}" for k, v in rubric["scores"].items()
    )
    ...
    ids = judge_tok(prompt, return_tensors="pt", truncation=True,
                    max_length=4096).input_ids.to(device)
    logits_list = []
    with torch.no_grad():
        out = judge_model.generate(
            ids, max_new_tokens=1024, do_sample=True,
            temperature=1.0, top_p=0.9,
            return_dict_in_generate=True, output_scores=True,
            pad_token_id=judge_tok.eos_token_id,
        )
    # 점수 토큰(1~5) 확률 가중합
    score_ids = {str(i): judge_tok.encode(str(i), add_special_tokens=False)[0] for i in range(1, 6)}
    ...
    soft = sum(i * probs[tid].item() for i, tid in [(int(k), v) for k, v in score_ids.items()])
...
def main(args):
    ...
    print("[1] 데이터 로드...")
    with open(SPLIT_PKL, "rb") as f: splits = pickle.load(f)
    with open(VITAL_MAP, "rb") as f: vital_map = pickle.load(f)
    ...
    for gen_model_id in SYNTH_MODELS:
        ...
        for n_shot in [2, 4]:
            ...
            for idx, row in tqdm(sft_df.iterrows(), total=len(sft_df)):
                ...
                s_brev = score_response(j_model, j_tok, emr_text, response, ref, "brevity", device)
                s_crit = score_response(j_model, j_tok, emr_text, response, ref, "critical_focus", device)
