...
- `[유의] 유의 실혈` — > 10% EBV (급성 실혈 >10% EBV)
- `[유의] 대량 실혈` — > 50% EBV (총 실혈 >50% EBV)
...
`0` 제거 외에 **물리적으로 불가능하거나 센서 이탈로만 설명되는 값**만 버린다
(`PLAUSIBLE_RANGE`). 배제 건수는 `DROPPED_ARTIFACTS`에 남고
`scripts/rebuild_vital_map.py`가 출력한다.

| 항목 | 유지 범위 | 비고 |
|---|---|---|
| HR | 1–300 | PALS 빈맥 상한 220 초과분은 남기고 300 초과만 배제 |
| SBP/DBP/MBP (침습 포함) | 5–300 | <5 = 커프·라인 이탈 |
| SpO2 | 1–100 | >100 은 정의상 불가 |
| T1 | 14–45 °C | **DHCA 18–20 °C 는 보존** |
| QTc | 150–800 ms | |
| Ppeak | 1–120 cmH₂O | |

> **왜 좁게 잡나.** 이 코호트는 소아 심장수술을 포함한다. 심폐순환우회·의도적
> 저체온에서 체온이 18~20 °C까지 내려가고, 청색성 심질환·단심실에서 SpO2 40~70%가
> 실제로 관측된다. "임상적으로 드문 값"을 지우면 **정작 인계해야 할 소견을 삭제**한다.
> 애매한 값은 남긴다 — 남기면 이벤트로 보고될 뿐이지만, 삭제는 되돌릴 수 없다.
