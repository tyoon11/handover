...
from config import (
    GOLD_PKL, VITAL_MAP_PKL, EVAL_OUT, EVAL_JUDGE_MODEL,
...
def judge_score(model, tokenizer, instruction: str, response: str, rubric: str) -> float:
...
def evaluate(args):
    ...
    with open(out_file, "w", encoding="utf-8") as fout:
        for rec in tqdm(results):
            ...
            b_score = judge_score(judge_model, judge_tok, instruction, gen, BREVITY_RUBRIC)
            c_score = judge_score(judge_model, judge_tok, instruction, gen, CRITICAL_RUBRIC)
